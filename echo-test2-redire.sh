...
echo -e "\\e[33m >>>>> This is for cp nginx file <<<<<<<<< \\e[0m" | tee -a /tmp/roboshop.log
...
echo -e "\e[33m >>>>>>>>> this is for nodejs setup <<<<<<<<<<<< \e[0m" | tee -a /tmp/roboshop.log
curl -fsSL https://rpm.nodesource.com/setup_20.x | bash -  &>>/tmp/roboshop.log
echo -e '\e[33m >>>>>>>>>> this is for install nodejs setup <<<<<<<<<<<<<< \e[0m' | tee -a /tmp/roboshop.log
dnf install -y nodejs &>>/tmp/roboshop.log

echo -e "\e[33m >>>>>>>>>>> this is for install frontend setup <<<<<<<<<<<< \e[0m" | tee -a /tmp/roboshop.log
